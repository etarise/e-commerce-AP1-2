...
const getCartItemsByCartId = (request, response) => {
  ...
  const createCartItem = (request, response) => {
    ...
    pool.query('INSERT INTO cartitems (cartitemid,productid,qty) VALUES ($1, $2, $3) RETURNING *', [cartid,productid,qty], (error, result) => {
      if (error) {
        throw error
      }
  ...
  const updateCartItem = (request, response) => {
    ...
    pool.query(
      'UPDATE cartitems SET qty = $1  WHERE cartitemid = $2',
      [qty,id],
      (error, results) => {
        if (error) {
          throw error
  ...
  const deleteCartItem = (request, response) => {
    ...
    pool.query('DELETE FROM cartitems WHERE cartitemid = $1', [id], (error, results) => {
      if (error) {
        throw error
      }
